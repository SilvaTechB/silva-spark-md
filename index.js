...
function wipeSession(reason) {
    try {
        ...
        // Also delete the creds.json specifically
    ...
    try {
        ...
        if (fs.existsSync(credsPath)) {
            try {
                ...
                // Check if session is older than 30 days (WhatsApp sessions typically expire)
    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (qr) {
                ...
                // Reset reconnection state when QR is shown
            ...
            if (connection === 'close') {
                ...
                if (statusCode === 405) {
                    ...
                    if (consecutive405 >= MAX_CONSECUTIVE_405) {
                        // Wipe session and force QR re-pairing
                        wipeSession('repeated 405 Connection Failure')
                        consecutive405 = 0
                        reconnectAttempts = 0
                        isReconnecting = false;
                        // Attempt reconnection with clean session
                ...
                // Send welcome message (text only to avoid 404 errors)
                try {
