const { proto, downloadContentFromMessage, getContentType, generateWAMessage, generateWAMessageFromContent } = require('@whiskeysockets/baileys')
const fs = require('fs')
...
const sms = (conn, m, store) => {
    try {
        ...
        if (m.message) {
            ...
            if (m.quoted) {
                ...
                if(quoted.viewOnceMessageV2) {
                    ...
                    m.quoted.forwardMessage = (jid, forceForward = true, options = {}) => conn.copyNForward(jid, vM, forceForward, {contextInfo: {isForwarded: false}}, options)
        ...
        m.reply = async (content, opt = { packname: "Silva Spark", author: "Silva Tech Inc" }, type = "text") => {
            try {
                switch (type.toLowerCase()) {
                    ...
                    case "sticker": {
                        ...
                        if (mime == "image/webp") {
                            ...
                            if (mime === "video") {
                                await conn.sendVideoAsSticker(m.chat, content, opt);
                            } else if (mime === "image") {
                ...
                // Fallback to simple text if reply fails
                try {
                    await conn.sendMessage(m.chat, { text: typeof content === 'string' ? content : 'Error sending message' }, { quoted: m });
                } catch (fallbackError) {
        ...
        m.senddoc = (doc, type, id = m.chat, option = {}) => conn.sendMessage(id, { 
            document: doc, 
            mimetype: type,
            fileName: option.filename || 'document',
            contextInfo: {
